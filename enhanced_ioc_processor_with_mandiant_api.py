#!/usr/bin/env python3
"""
IOC to Excel Template Processor - Enhanced with Mandiant API Integration
- Fetches report titles from Mandiant API based on report numbers
- Adds report titles to the notes section in the compiled output
...
# Mandiant API Settings
MANDIANT_API_KEY = ""  # Your Mandiant Advantage API Key
MANDIANT_SECRET_KEY = ""  # Your Mandiant Advantage Secret Key
MANDIANT_AUTH_URL = "https://api.intelligence.mandiant.com/v4/auth"
MANDIANT_BASE_URL = "https://api.intelligence.mandiant.com/v4"
MANDIANT_RATE_LIMIT = 0.5  # Seconds between API requests
MANDIANT_CACHE_FILE = "mandiant_report_cache.json"  # Cache file for report titles
MANDIANT_CACHE_EXPIRY_DAYS = 30  # How long to cache report titles
...
print(f"  Mandiant API: {'Enabled' if MANDIANT_API_KEY and MANDIANT_SECRET_KEY else 'Disabled'}")
...
# =============================================================================
# CELL 3: Mandiant API Functions
# =============================================================================
print("\nCELL 3: Setting up Mandiant API Functions")
print("=" * 50)

class MandiantAPIClient:
    """
    Client for interacting with Mandiant Advantage API
    """
    def __init__(self, api_key, secret_key):
        ...
        self.report_cache = self.load_cache()
        
    def load_cache(self):
        """Load cached report titles from file"""
        if os.path.exists(MANDIANT_CACHE_FILE):
            try:
                with open(MANDIANT_CACHE_FILE, 'r') as f:
                    cache = json.load(f)
                    # Clean expired entries
                    cutoff_date = datetime.now().timestamp() - (MANDIANT_CACHE_EXPIRY_DAYS * 86400)
                    cache = {k: v for k, v in cache.items() 
                            if v.get('timestamp', 0) > cutoff_date}
                    return cache
            except:
                return {}
        return {}
    ...
    def save_cache(self):
        ...
        try:
            with open(MANDIANT_CACHE_FILE, 'w') as f:
                json.dump(self.report_cache, f, indent=2)
        except Exception as e:
            print(f"    ⚠ Could not save cache: {e}")
    
    def authenticate(self):
        """Get authentication token from Mandiant API"""
        ...
        try:
            ...
            if response.status_code == 200:
                ...
                self.token_expiry = datetime.now().timestamp() + expires_in - 60  # Buffer
    ...
    def get_report_title(self, report_id):
        """
        Fetch report title from Mandiant API
        
        Args:
            report_id: The report identifier (e.g., "25-00012345")
            
        Returns:
            Report title string or None if not found
        """
        # Check cache first
        if report_id in self.report_cache:
            cached = self.report_cache[report_id]
            print(f"    ✓ Using cached title for {report_id}")
        ...
        try:
            ...
            if '-' in report_id:
                numeric_id = report_id.split('-')[-1]
            
            # Try different API endpoints based on report type
            endpoints = [
                f"/reports/{numeric_id}",
                f"/finished-intelligence/{numeric_id}",
                f"/threat-intelligence/reports/{numeric_id}"
            ]
            ...
            for endpoint in endpoints:
                url = MANDIANT_BASE_URL + endpoint
                ...
                if response.status_code == 200:
                    ...
                    if title:
                        ...
                        self.save_cache()
                        print(f"    ✓ Found title for {report_id}: {title[:50]}...")
                        time.sleep(MANDIANT_RATE_LIMIT)  # Rate limiting
                        return title
                
                elif response.status_code != 404:
                    print(f"    ⚠ API error for {report_id}: {response.status_code}")
        ...
        time.sleep(MANDIANT_RATE_LIMIT)  # Rate limiting even on failure
        return None
    
    def extract_report_ids_from_filename(self, filename):
        """
        Extract report IDs from filename
        Common patterns: 25-00012345.csv, CSA-25-00012345.csv, etc.
        """
        report_ids = []
        
        # Pattern for Mandiant report numbers
        patterns = [
            r'(\d{2}-\d{8})',  # 25-00012345
            r'(\d{2}-\d{7})',   # 25-0012345
            r'(\d{2}-\d{6})',   # 25-001234
        ]
        
        for pattern in patterns:
            matches = re.findall(pattern, filename)
            report_ids.extend(matches)
        
        return report_ids

# Initialize Mandiant client if configured
mandiant_client = None
if MANDIANT_API_KEY and MANDIANT_SECRET_KEY:
    mandiant_client = MandiantAPIClient(MANDIANT_API_KEY, MANDIANT_SECRET_KEY)
    print("✓ Mandiant API client initialized")
else:
    print("⚠ Mandiant API disabled (no credentials configured)")

# =============================================================================
# CELL 4: Validate Configuration and Paths
# =============================================================================
print("\nCELL 4: Validating Configuration")
print("=" * 50)
...
# =============================================================================
# CELL 5: Define Excel Handler Functions
# =============================================================================
print("\nCELL 5: Defining Excel Handler Functions")
print("=" * 50)
...
# =============================================================================
# CELL 6: Define Password Protection Functions
# =============================================================================
print("\nCELL 6: Defining Password Protection Functions")
print("=" * 50)
...
# =============================================================================
# CELL 7: Define Enhanced IOC Processing Functions for Mandiant
# =============================================================================
print("\nCELL 7: Defining Enhanced Mandiant IOC Processing Functions")
print("=" * 50)

def process_mandiant_csv(file_path):
    """
    Process Mandiant CSV file (25-*.csv format) with API enrichment.
    """
    iocs = []
    
    # Extract report IDs from filename for API lookup
    filename = os.path.basename(file_path)
    report_title = None
    
    if mandiant_client:
        report_ids = mandiant_client.extract_report_ids_from_filename(filename)
        if report_ids:
            print(f"  Found report ID(s) in filename: {report_ids}")
            for report_id in report_ids:
                report_title = mandiant_client.get_report_title(report_id)
                if report_title:
                    print(f"  ✓ Retrieved report title: {report_title[:60]}...")
                    break
    ...
    try:
        with open(file_path, 'r', encoding='utf-8', errors='ignore') as f:
            ...
            for row in reader:
                ...
                # Build the note field with report title if available
                note_parts = []
                
                # Add original note if present
                if row.get('Note', '').strip():
                    note_parts.append(row.get('Note', '').strip())
                
                # Add report title if found
                if report_title:
                    note_parts.append(f"Report: {report_title}")
                
                # Add report ID if no title was found but ID exists
                elif report_ids and not report_title:
                    note_parts.append(f"Report ID: {report_ids[0]}")
...
print("✓ Enhanced Mandiant processing function defined")

# =============================================================================
# CELL 8: Define IOC Processing Functions for CrowdStrike
# =============================================================================
print("\nCELL 8: Defining CrowdStrike IOC Processing Functions")
print("=" * 50)
...
# =============================================================================
# CELL 9: Optional - Define VirusTotal Enrichment Function
# =============================================================================
print("\nCELL 9: Defining VirusTotal Enrichment Function (Optional)")
print("=" * 50)
...
# =============================================================================
# CELL 10: Scan for IOC Files
# =============================================================================
print("\nCELL 10: Scanning for IOC Files")
print("=" * 50)
...
# =============================================================================
# CELL 11: Extract IOCs from All Files with Mandiant API Enrichment
# =============================================================================
print("\nCELL 11: Extracting IOCs from Files with API Enrichment")
print("=" * 50)

all_iocs = []
report_titles_found = defaultdict(list)  # Track which reports we found titles for

# Process Mandiant files with API enrichment
if mandiant_files:
    print("\nProcessing Mandiant files with API enrichment:")
    for csv_file in mandiant_files:
        iocs = process_mandiant_csv(str(csv_file))
        all_iocs.extend(iocs)
        
        # Track report titles for summary
        for ioc in iocs:
            note = ioc.get('Note:', '')
            if 'Report:' in note:
                report_titles_found[csv_file.name].append(note)
...
if all_iocs:
    ...
    # Show Mandiant report title enrichment summary
    if report_titles_found:
        print("\nMandiant Report Title Enrichment:")
        for filename, titles in report_titles_found.items():
            if titles:
                unique_title = titles[0].split('Report: ')[1] if 'Report:' in titles[0] else titles[0]
                print(f"  • {filename}: {unique_title[:60]}...")
    
    # Show sample IOC with report title
    print("\nSample IOC entry with report title:")
    for ioc in all_iocs:
        if 'Report:' in ioc.get('Note:', ''):
...
# =============================================================================
# CELL 12: Prepare Output File
# =============================================================================
print("\nCELL 12: Preparing Output File")
print("=" * 50)
...
# =============================================================================
# CELL 13: Open Excel Template and Add IOCs with Enriched Notes
# =============================================================================
print("\nCELL 13: Processing Excel Template with Enriched IOCs")
print("=" * 50)
...
if output_prepared and all_iocs:
    try:
        ...
        print(f"\nAdding {len(all_iocs)} IOCs to Excel (with report titles)...")
        ...
        if success:
            # Save the workbook
            print("\nSaving Excel file with enriched data...")
...
# =============================================================================
# CELL 14: Apply Password Protection
# =============================================================================
print("\nCELL 14: Applying Password Protection")
print("=" * 50)
...
if os.path.exists(OUTPUT_EXCEL_PATH) and all_iocs:
    ...
    # Mandiant API enrichment summary
    if mandiant_client and report_titles_found:
        print(f"\n  Mandiant API Enrichment:")
        print(f"    Report titles fetched: {len(report_titles_found)}")
        print(f"    Cache saved: {MANDIANT_CACHE_FILE}")
...
# =============================================================================
# CELL 16: Quick Test - Verify Output (Optional)
# =============================================================================
print("\nCELL 16: Output Verification (Optional)")
print("=" * 50)
            ...
            if EXCEL_SHEET_NAME in workbook.sheetnames:
                ...
                for row in range(EXCEL_DATA_START_ROW, ws.max_row + 1):
                    if any(ws.cell(row=row, column=col).value for col in range(1, ws.max_column + 1)):
                        data_rows += 1
                        # Check for report titles in notes
                        for col in range(1, ws.max_column + 1):
                            cell_value = ws.cell(row=row, column=col).value
                            if cell_value and 'Report:' in str(cell_value):
                                has_report_title = True
                                break
                
                print(f"  Data rows found: {data_rows}")
                print(f"  Contains report titles: {'Yes' if has_report_title else 'No'}")
                
                # Show first few IOCs with report titles
                print(f"  Sample data from row {EXCEL_DATA_START_ROW}:")
                sample_shown = False
                for row in range(EXCEL_DATA_START_ROW, min(EXCEL_DATA_START_ROW + 5, ws.max_row + 1)):
                    for col in range(1, ws.max_column + 1):
                        header = ws.cell(row=EXCEL_HEADER_ROW, column=col).value
                        value = ws.cell(row=row, column=col).value
                        if header and value and 'Note' in str(header) and 'Report:' in str(value):
                            print(f"    {header}: {str(value)[:80]}...")
                            sample_shown = True
                            break
                    if sample_shown:
                        break
...
print("\n" + "=" * 50)
print("Enhanced IOC Processor with Mandiant API Integration - Complete")
print("=" * 50)
