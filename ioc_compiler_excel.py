...
def add_iocs_to_excel(workbook, sheet_name, iocs):
    """
    Add IOCs to an existing Excel worksheet.
    Assumes the first row contains headers matching the IOC dictionary keys.
    """
    try:
        # Get the worksheet
        ...
        # If the sheet only has headers (row 1), start from row 2
        if ws.max_row == 1:
            next_row = 2
        
        # Check if there's actual data in the last row (sometimes max_row includes empty rows)
        while next_row > 1:
            has_data = False
            for col in range(1, len(headers) + 1):
                if ws.cell(row=next_row-1, column=col).value:
                    has_data = True
                    break
            if has_data:
                break
        ...
        if next_row == 1:  # Only headers exist
            next_row = 2
...
def compile_iocs_to_excel(folder_path, excel_path, password, sheet_name):
    """
    ...
    if all_iocs:
        ...
        try:
            ...
            success = add_iocs_to_excel(workbook, sheet_name, all_iocs)
...
if __name__ == "__main__":
    ...
    # Run the compiler
    try:
        compile_iocs_to_excel(IOC_FOLDER, EXCEL_FILE_PATH, EXCEL_PASSWORD, EXCEL_SHEET_NAME)
    except Exception as e:
        print(f"\n❌ UNEXPECTED ERROR: {e}")
